...
if __name__ == '__main__':
    ...
    parser.add_argument('--step2_weights_path', dest='step2_weights_path', type=str,
                        help='Path to the h5 file holding the detector weights from step 2.'
                             'Must be compatible with the network. Leaving this out defaults to imagenet weights.',
                        default=None)
    ...
    if args.network == 'vgg16':
        rpn_base = vgg.vgg16_base(freeze_blocks=[1, 2, 3, 4, 5],
                                  weight_regularizer=vgg.WEIGHT_REGULARIZER, bias_regularizer=vgg.BIAS_REGULARIZER)
        rpn_model = vgg.vgg16_rpn(rpn_base, weight_regularizer=vgg.WEIGHT_REGULARIZER,
        ...
        rpn_base = resnet.resnet50_base(freeze_blocks=[1, 2, 3, 4])
        rpn_model = resnet.resnet50_rpn(rpn_base, weight_regularizer=resnet.WEIGHT_REGULARIZER,
        ...
        rpn_base = resnet.resnet101_base(freeze_blocks=[1, 2, 3, 4])
    ...
    if args.save_weights_dest:
        ...
        save_weights_dest = "models/rpn_weights_{}_step3.h5".format(args.network)
    if args.save_model_dest:
        save_model_dest = args.save_model_dest
    else:
        save_model_dest = "models/rpn_model_{}_step3.h5".format(args.network)

    if args.step2_weights_path is not None:
        rpn_model.load_weights(args.step2_weights_path, by_name=True)
